...
The two original replicates do not have the same baseline duration.
To include them in the same analysis, we decided to express time relative to the experimental perturbation:
...
It is important to visualise an overview of how each individual behaved and, if necessary, alter the data accordingly.
...
result_dt[, lapply(.SD, function(x) 
