...
The study of circadian rhythm, employing fruit flies as an animal model, is a well established research field recently awarded {This is not atemporal, in 10 years will still say recently} with the Nobel Prize in Physiology or Medicine.
...
\texttt{R} provides one of the richest statistical toolboxes available, which allows users to go deeper in the analysis of the extracted variables.
...
result_dt <- summary_dt[,
          .(
            mean_period = round(mean(first_peak_period, na.rm = T) / hours(1), 2),
            sd_period = round(sd(first_peak_period, na.rm = T) / hours(1), 2),
            percent_rhythmic = round(100 * sum(is_rhythmic) / .N, 2),
...
This corresponds to the position of animals walking along (back and forward) their tubes in a very paced manner.
