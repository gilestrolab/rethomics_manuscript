...
Fist, we install the \texttt{rethomics} packages (see the webpage for installation instructions) and download the data to extract the zip archive in our working directory.
...
MUTANT_PALETTES <- c("#babdb6", "#729fcf")
