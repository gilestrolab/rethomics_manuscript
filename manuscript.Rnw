...
In the 1960s, behavioural geneticists, such as Seymour Benzer showed that some apparently complex behaviours could be in fact governed by simple genetic determinant,
which breached the gap between genetics and the study of behaviour\citationneeded{}.
...
which is deeply transforming both genetics\citationneeded{} and neurobiology\citationneeded{}.
...
It is now accepted that the study of behaviour can also benefit from quantitative sciences such as mathematics, physics and computational linguistics \cite{brown_ethology_2018, berman_measuring_2018}.
...
to score more complex ones such as feeding\cite{itskov_automated_2014,ro_flic_2014}, aggression \citationneeded{}, and courtship\cite{tsai_image_2012};
...
At the core of \texttt{rethomics} lies the \texttt{behavr} data structure, an object used to store large amounts data (\emph{e.g.} position and activity) and metadata (\emph{e.g.} treatment and genotype) in a unique \texttt{data.table}-derived object\cite{dowle_data.table_2017}.
