...
However, there is little effort (I would not say little effort, I will rephrase it, making a genereic framework is not easy or 
what people is focus on) in providing a generic framework to specifically analyse multiple and long behavioural time series.
(some conector with previos sentence, for that reason, that is why...) We developed the \texttt{rethomics} framework, 
...
The behaviour of an animal is a complex phenotypical manifestation of the interaction between its nervous system and external or internal (input signals) environments.
...
The availability of large amounts of data paves the way for in-depth quantitative analyses.
...
is not trivial  (why is not trivial, explain, for example: due to large datasets, memory problems,...).
...
\texttt{rethomics} offers an elegant (I will remove subjective opinions like elegant) computational solution to store, manipulate and visualise large amounts of data.
We see it as a tool to fill the gap between behavioural biology and data sciences, thus promoting collaboration between computational and behavioural scientists.
...
It is interesting to record multiple metavariables in so they can later be used as covariates. 
