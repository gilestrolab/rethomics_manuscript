...
The multiplicity of model organisms, hypotheses and paradigms makes the existence of a diverse palette of recording tools important.
...
Since many aspect of behaviour analysis are becoming increasingly linked to data sciences, the development of common tools and data structures would be valuable to the community.

At first, it may seem as though behavioural experiments are prohibitively heterogeneous -- in terms of model organisms, paradigm and time scale -- for an analogous ecosystem to arise.
...
In the article herein, we describe the \texttt{rethomics} platform, a collection of packages offering a solution to import, store, manipulate and visualise large amounts of behavioural results
in a data mining and statistical framework.
We also present two examples of application of this tool to the analysis of circadian rhythm in fruit flies, a widely studied behaviour.
