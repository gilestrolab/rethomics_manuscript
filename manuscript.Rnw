...
A summary of the functionality of all packages is represented in a supplementary table (\nameref{S1-Table}).
...
We showed that \texttt{rethomics} can readily be used to analyse specifically circadian data. As such, it already fills a clear gap in the sleep and circadian community by providing an open, extensible and programmatic toolbox as opposed to the rigid array of GUI tools which cannot interoperate.
In particular, within the \texttt{R} environment, \texttt{rethomic} features a streamlined workflow (Fig.~\ref{fig:fig-1}):
data loading; quality control (Fig.~\ref{fig:fig-3}); per individual visualisation (Fig.~\ref{fig:fig-4}A) and computation (Fig.~\ref{fig:fig-4}B); group averages and statistics (Fig.~\ref{fig:fig-5}), ultimately resulting in publication-quality figures.
In addition, to the canonical analysis we demonstrated, the richness of the \texttt{R} environment renders rethomics readily available for exploratory work such as the use of continuous wavelet transform to study the modulation of walking pace(Fig.~\ref{fig:fig-6}).

Altogether, our tool provides a needed opportunity for the community to share analytic and visualisation tools whilst embracing the diversity of recording equipment. By design, rethomics is very flexible and not limited to the study of circadian rhythms.
In fact, \texttt{behavr} tables (Fig.~\ref{fig:fig-2}) are a general solution to link data to metadata that come with no assumption regarding the type and number of variables stored. Therefore, we anticipate it will apply to  a variety contexts and scales.
