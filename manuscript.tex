...
In order to compute CWT, we interfaced the \texttt{WaveletComp} package\cite{schmidbauer_waveletcomp_2018} in \texttt{rethomics}. 
This interface allowed us to average the result of the five consecutive days in the time/period domain over one circadian day both for the two representative animals (Fig~\ref{fig:fig-6}D)
...
We showed that \texttt{rethomics} can readily be used to analyse circadian data. As such, it already fills a clear gap in the sleep and circadian community by providing an open, extensible and programmatic toolbox as opposed to a rigid array of GUI tools which cannot interoperate.
In particular, within the \texttt{R} environment, \texttt{rethomic} features a streamlined workflow (Fig.~\ref{fig:fig-1}):
data loading; quality control (Fig.~\ref{fig:fig-3}); per individual visualisation (Fig.~\ref{fig:fig-4}A) and computation (Fig.~\ref{fig:fig-4}B); group averages and statistical analysis (Fig.~\ref{fig:fig-5}), ultimately resulting in publication-quality figures.
In addition to the canonical analysis we presented in this study, the richness of the \texttt{R} environment renders rethomics particularly interesting for exploratory work such as the use of continuous wavelet transform to study the modulation of walking pace(Fig.~\ref{fig:fig-6}).

Altogether, our tool provides a needed opportunity for the community to share analytic and visualisation solutions whilst embracing the diversity of recording equipment. By design, rethomics is very flexible and not limited to the study of circadian rhythms.
...
For instance, in the last decade, multiple devices have been developed -- either commercially or academically -- to record longitudinal data on several animal models (\emph{i.e.} feeding, metabolic activity, heart rate, position and so on). In collaboration with the teams providing of these methods, we envisage the development of device-specific packages that will load data into \texttt{rethomics}, making visualisation and analysis immediately available. 
For instance, the developer any behaviour recording method can simply write a small package to read their data in a \texttt{behavr} table and benefit from the functionalities available.
Furthermore, such an approach becomes particularily useful in order to compare methods or study behaviours that are recorded by different devices, on the same individuals. Indeed, in these contexts, it becomes apparent that analysis must be unified at some level.

In conclusion, \texttt{rethomics} already delivers state-of-the-art tools to study circadian rhythm using conventional tools, but was designed in an inclusive and extensible manner. As such, if fills a unique niche in a community that is becoming increasingly quantitative.
