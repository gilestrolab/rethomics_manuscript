...
Instead, tools tend to consist of graphical interfaces that import data from a single platform and provide somewhat rigid functionalities.
...
Efficiently combining and manipulating metadata and data from hundreds of individuals, each recorded for days or weeks, is not trivial.
...
Currently, we provide a package to read single or multi-beam Drosophila Activity Monitor (DAM)\citationneeded{} data and another one for Ethoscope data\cite{geissmann_ethoscopes:_2017}.
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in FUN(X[[i]], ...): object 'first\_peak\_rel\_power' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in FUN(X[[i]], ...): object 'first\_peak\_rel\_power' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in cowplot::plot\_grid(fig5, legend, nrow = 2, rel\_heights = c(15, : object 'fig5' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in print(fig5\_final): object 'fig5\_final' not found}}\end{kframe}
\end{knitrout}
