...
We developed the \texttt{rethomics} framework, a suite of \texttt{R} packages that altogether offers utilities to:
...
The scoring of behaviours is certainly not an exception to this trend.
...
The multiplicity of model organisms, hypotheses and paradigms makes the existence of diverse range of recording tools important.
...
Instead, tools tend to consist of graphical interfaces that tend to import data from a single platform and provide somewhat rigid functionalities.
...
Efficiently combining and manipulating metadata and data of hundreds of individuals, each recorded for days or weeks, is not trivial.

In the article herein, we describe \texttt{rethomics}, a framework that unifies analysis of behavioural dataset in an efficient and flexible manner. We implemented it in \texttt{R}\cite{r_core_team_r:_2017} since it is widely taught and adopted in the community of computational biologists with a vast number of open-source packages.
\texttt{rethomics} offers an elegant computational solution to store, manipulate and visualise  large amounts of data.
...
At the core of \texttt{rethomics}, the \texttt{behavr} package offers a very flexible and efficient solution to store large amounts data (\emph{e.g.} position and activity) as well as metadata (\emph{e.g.} treatmentand genotype) in a single \texttt{data.table}-derived object\cite{dowle_data.table:_2017}.
...
We created \texttt{behavr} (Fig~\ref{fig:fig-2}A), a new data structure, based on the widely adopted \texttt{data.table} object, in order to address two challenges that are inherent to manipulating ethomics results.
...
It is interesting to record multiple metavariables in so far as they so they can later be used as covariates. 
...
For instance, when one wants to update variable according to the value of a metavariable (say, alter the variable $x$ only for animals with the metavariable $sex=``male"$). The online tutorials and documentation provide a detailed set of examples and concrete use cases of \texttt{behavr}. 
...
Currently, we provide a package to read Drosophila Activity Monitor (DAM2)\citationneeded{} data and another one for Ethoscope data\cite{geissmann_ethoscopes:_2017}.
...
It explicitly lists all treatments and individuals, which facilitates interspersion of treatments (indeed, without it, users are tempted to simplify their experimental design by, for instance, confounding device/location and treatment).
...
We gathered a subset of the data from a recent publication \cite{buhl_quasimodo_2016}, which was gracefully made publicly available by the authors\citationneeded{zenodo}.
...
The authors have recorded the activity of both TIM/NKCCOK (NKCCox is expressed in their clock neurons) and NKCCOK/+ (the appropriate control) in LL for six days.
...
Several methods are implemented in \texttt{zeitbebr} ($\chi{}^2$, Lomb-Scargle and autocorrelation).
In this example, we generate $\chi{}^2$ periodograms and lay them out in a grid.
Periodograms dor the subset of eight animals used in Fig~\ref{fig:fig-4}A is shown in Fig~\ref{fig:fig-4}B. 
...
As shown in the original study, double-plotted actograms and periodograms suggest that NKCCOX/+ flies are mostly 
...
To further quantify this difference, we can show the number of rhythmic animals -- \emph{i.e.} individuals for which a peak was found -- in each groups (see Fig~\ref{fig:fig-5}B).
...
\hlstd{summary_dt} \hlkwb{<-} \hlstd{per_dt[,}\hlkwd{.}\hlstd{(}
                        ...
                        \hlkwc{first_peak_rel_power} \hlstd{= power[peak}\hlopt{==}\hlnum{1}\hlstd{]} \hlopt{-} \hlstd{signif_threshold[peak}\hlopt{==}\hlnum{1}\hlstd{],}
...
This workflow applies  particularly really well to much larger data sets and provides a large degree of flexibility taht will allow
biologists to tune their analysis to their specific questions.
...
Marcus Gosh, Patrick Kr{\"a}tschmer Rob
