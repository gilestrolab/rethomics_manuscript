...
The multiplicity of model organisms, hypotheses and paradigms justifies the existence of a diverse palette of recording techniques important.
...
We also present two practical examples of its application to the analysis of circadian rhythm in fruit flies, a widely studied behaviour.
...
Visualisation and other results are standard objects, and can therefore be further analysed inside the wide \texttt{R} package ecosystem.
...
It explicitly lists all treatments and individuals, which facilitates interspersion of conditions (without it, users are tempted to simplify their experimental design by, for instance, confounding device/location and treatment).
...
\texttt{ggetho} makes full use of the internal \texttt{behavr} structure to summarise of temporal trends.
...
The second one shows how \texttt{rethomics} integrates with the rest of \texttt{R} with an advanced case of a multi-scale analysis of a periodic behaviour using continuous wavelet transform and ethoscope data \cite{geissmann_ethoscopes:_2017}.
...
Indeed, two animals in different experiments may have the same label, but different \texttt{id}s.
...
For instance, an animal could have rhythmic bursts of activity recurring at high frequency (\emph{e.g.} 1~min), but also a circadian  (\emph{i.e.} 24~h) regulation of the same variable, activity.
...
that is, if the periodicity of a high frequency rhythm itself can be driven by a lower frequency one.
...
To illustrate how this analysis can be adapted to the study of behaviour, we studied the position of 40 males and 40 virgins females in their glass tubes.
The male data was collected from our previous study \cite{geissmann_ethoscopes:_2017} (controls in figure~5M-P).
We used the package \texttt{scopr}, part of \texttt{rethomics}, to load five days of ethoscope positional data, which we sampled it at 0.1 Hz.
...
\begin{figure}[!h]
	...
	\caption{{\bf Wavelet analysis of positional data.}
		A: Raw position data for a representative male (top) and virgin female (bottom) \emph{Drosophila} over five days, in black. 
		The thick coloured lines show the local average position.
		The green rectangles in the background shows the two time windows selected for B and C.
		B: Close up of A, showing position over one hour, in the beginning of the L phase ($t \in [24, 25]h$).
		C: Close up of A, showing position over one hour, in the middle of the L phase ($t \in [30, 31]h$).
		...
		In D and E, the lines on the right shows the marginal power spectra corresponding to the shown spectrograms (average on the period domain).
	}
	\label{fig:fig-6}
\end{figure}
...
We then averaged the result of the five consecutive days in the time/period domain over one circadian day (Fig~\ref{fig:fig-6}D~and~E).

As expected, we observe strong, high period (12~h and 24~h), rhythms of the position.
