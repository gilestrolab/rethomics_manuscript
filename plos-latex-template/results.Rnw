...
fig4A <- ggperio(per_dt, aes(y = power - signif_threshold,
                             colour = genotype)) + 
...
<<>>=
# standard ggplot
fig4B <- ggplot(summary_dt, aes(y = first_peak_period,
                                x = genotype)) + 
...
<<cache=TRUE, warning=FALSE, results="hide">>=
# standard ggplot
fig4C <- ggplot(summary_dt, aes(x = genotype,
                                fill = genotype,
                                alpha = is_rhythmic
                                )) +
...
fig4 <- cowplot::plot_grid(fig4A + theme(legend.position="none"),
